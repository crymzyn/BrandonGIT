...
instanceCI.addQuery('u_type','Analysis Services');
instanceCI.query();

while (instanceCI.next()) {

	instanceCI.sys_class_name = 'u_msft_sql_analysis_services';
	instanceCI.update();
...
var instance_old = new GlideRecord('u_msft_sql_analysis_services');
...
while (instance_old.next()) {
	...
	var instance_new = new GlideRecord('u_msft_sql_analysis_services');
	...
	if (instance_new.next()) {
		...
		sqlProcess = instance_new.running_process;
	...
	getClassifier.addQuery('name','Microsoft Analysis Services');
	getClassifier.query();
...
var instance_clean = new GlideRecord('u_msft_sql_analysis_services');
